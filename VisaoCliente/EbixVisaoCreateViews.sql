...
CREATE  FORCE VIEW "DB_EXCHANGE_VISAO_TST"."VW_RELPORTABILIDADE" ("ID_DOCUMENTO", "ANGARIADOR", "PARCERIA", "CANALVENDA", "PID_CANAL_VENDA", "PID_SITUACAO", "PID_EMPRESA", "DS_STATUS", "NM_CLIENTE", "IDUSUARIO", "CD_TIPO_PRODUTO", "ESTRUTURAVENDA", "PID_ESTRUTURA_VENDA", "IDPORTSTATUS", "DT_MOVIMENTO", "PID_DOCUMENTO_PORTABILIDADE", "DT_ATUALIZACAO", "SEGURADO", "CPF", "SEXO", "DTNASCIMENTO", "REGIMETRIBUTARIO", "SITUACAO", "PORTABILIDADE", "SEGURADORACEDENTE", "FUNDODESTINO", "FUNDOCEDENTE", "TIPOPLANOCEDENTE", "CERTIFICADOCEDENTE", "VALOR", "MOTIVORECUSA", "ESTAGIO", "PROPOSTA", "DTSOLICITACAO", "QTDDIAS", "DTCONCLUSAO", "ASSISTENTE", "PRODUTO", "SITUACAOXPORT") AS 
  SELECT 
         portabilidade.PID_DOCUMENTO_PORTABILIDADE as id_documento,
         usuario.NM_USUARIO AS angariador,
         ...
         FROM
             tb_documento doc 
         WHERE
             doc.id_documento = portabilidade.pid_documento_previdencia) AS proposta,
         (SELECT
             hd.dt_atualizacao 
         FROM
             tb_historico_documento hd 
         where
             hd.pid_situacao =3 
             and doc.id_documento = hd.pid_documento) AS dtSolicitacao,
          TRUNC((SELECT
             MIN(portstatus.dt_movimento) 
         FROM
             TB_PORTABILIDADE_STATUS)) - 
        TRUNC((SELECT
             hd.dt_atualizacao 
         FROM
             tb_historico_documento hd 
   ...
   --          hd.pid_situacao =3 
        ...
        (SELECT
             MIN (port.DT_MOVIMENTO) dt_inclusao 
         FROM
            tb_portabilidade_status port 
         where
             portabilidade.pid_documento_portabilidade = port.pid_portabilidade 
     ...
     INNER JOIN
         tb_documento doc 
            ...
            AND doc.pid_tipo_documento = 5 
     INNER JOIN
         tb_portabilidade portabilidade 
             ON doc.id_documento = portabilidade.pid_documento_portabilidade 
    INNER JOIN
         tb_seguradora seguradora 
             ON portabilidade.pid_seguradora_ced = seguradora.id 
     INNER JOIN
         tb_regime_tributario regime 
             ON portabilidade.pid_regime_tributario_ced = regime.id       
     INNER JOIN
         tb_empresa empresa 
     ...
     INNER JOIN
         tb_tipo_produto tipoproduto 
             ON portabilidade.pid_tipo_produto = tipoproduto.cd_tipo_produto 
     INNER JOIN
         tb_usuario usuario 
             ON doc.pid_usuario = usuario.id 
     INNER JOIN
         tb_documento_previdencia docprev 
             ON portabilidade.pid_documento_previdencia = docprev.pid_documento_previdencia 
    INNER JOIN
    ...
    INNER JOIN
     ...
     LEFT JOIN
         tb_usuario_canal_venda canalusuario 
             ON canalusuario.pid_usuario = usuario.id
     LEFT JOIN
     ...
     LEFT JOIN
         tb_portabilidade_status portstatus 
             ON portabilidade.pid_documento_portabilidade = portstatus.pid_portabilidade 
     LEFT JOIN
         TB_USUARIO usuario2 
             ON usuario2.ID =  DOC.PID_USUARIO_DIGITADOR  
     INNER JOIN
         TB_HISTORICO_DOCUMENTO hist_doc 
             ON doc.id_documento = hist_doc.pid_documento 
             AND hist_doc.pid_situacao = (
                 SELECT
                     MAX(sit2.pid_situacao) 
             FROM
                 tb_historico_documento sit2 
             WHERE
                 sit2.pid_documento = hist_doc.pid_documento
         ) 
     INNER JOIN
         TB_SITUACAO  situacao 
             ON situacao.id = hist_doc.PID_SITUACAO 
     WHERE
         1=1 
        AND  portstatus.id = (
                 select
                 ...
                 FROM
                     tb_portabilidade_status port 
                 where
                     port.pid_portabilidade = portabilidade.pid_documento_portabilidade
             );
CREATE  FORCE VIEW "DB_EXCHANGE_VISAO_TST"."VW_RELPROPOSTA" ("PID_CANAL_VENDA", "PID_SITUACAO", "IDHISTORICO", "PID_EMPRESA", "NM_CLIENTE", "IDUSUARIO", "CD_TIPO_PRODUTO", "DS_SITUACAO", "PID_ESTRUTURA_VENDA", "DT_ATUALIZACAO", "DT_CARGA", "ID_CARGA", "ANGARIADOR", "PARCERIA", "CANALVENDA", "ESTRUTURAVENDA", "PRODUTO", "SEGURADO", "CPF", "SEXO", "DTNASCIMENTO", "PROPOSTA", "CERTIFICADO", "NOMEFUNDO", "TIPOPLANO", "REGIMETRIBUTARIO", "TITULARIDADE", "FORMAPAGAMENTO", "PREMIOTOTAL", "VALORCONTRIBUICAOREGULAR", "PRIMEIROAPORTE", "ASSISTENTE", "DTSITUACAO", "SITUACAO") AS 
  SELECT
        doc.pid_canal_venda,
         hist_doc.PID_SITUACAO as PID_SITUACAO,
...
GRANT SELECT ON "VW_PROPOSTA" TO "CS_EXCHANGE_VISAO_HML";
GRANT SELECT ON "VW_PORTABILIDADE" TO "CS_EXCHANGE_VISAO_HML";
GRANT SELECT ON "VW_RELPORTABILIDADE" TO "CS_EXCHANGE_VISAO_HML";
GRANT SELECT ON "VW_RELPROPOSTA" TO "CS_EXCHANGE_VISAO_HML";
GRANT SELECT ON "VW_RELCARTEIRA" TO "CS_EXCHANGE_VISAO_HML";
