...
-- 03 - POPULA - TB_TIPO_MOTIVO_RECUSA
SELECT 'INSERT INTO TB_TIPO_MOTIVO_RECUSA (ID,DS_MOTIVO) VALUES ('||ID||','''||DS_MOTIVO||''');'
FROM WISGCICATU_PRD.TB_TIPO_MOTIVO_RECUSA
.
spool Insert_TB_TIPO_MOTIVO_RECUSA.sql
/
spool off
...
-- 38 - POPULA - TB_ARQUIVO_PORTABILIDADE
...
-- 39 - POPULA - TB_ARQUIVO_PAGAMENTO
...
/
spool off;
/*   COMENTADO POIS SERAO CARREGADAS POR DATAPUMP DO AMBIENTE PRODUTIVO

-- 40 - POPULA - TB_ARQUIVO_MOVI
SELECT 'INSERT INTO TB_ARQUIVO_MOVI (ID,PID_ARQ_STATUS,TP_REGISTRO,DS_HISTORICO,NU_PROPOSTA,NU_MATRICULA,NU_CERTIFICADO,NM_FUNDO,CD_FUNDO,CT_FUNDO,DT_MOVIMENTO,QT_COTAS,VL_MOVIMENTO,DT_COTA,VL_COTA,VL_DA,VL_IR,NU_DOCUMENTO,DT_PAGAMENTO,NU_PARCELA,VALOR_TAXA_SAIDA,DS_OPERACAO_REALIZADA,CD_ANGARIADOR,DS_AGENCIA,VL_IDENTIFICACAO_CLIENTE,NM_PRODUTO,CD_PLANO_PARCEIRO,NU_PROTOCOLO,VL_NOMINAL,DS_CGC_CPF_CLIENTE,CD_MOVIMENTO_ICATU,DT_SOLICITACAO,NU_PROC_SUSEP,DS_SINAL_MOVIMENTACAO,NU_CONTRATO) VALUES ('||ID||','||PID_ARQ_STATUS||','||TP_REGISTRO||','''||DS_HISTORICO||''','||NU_PROPOSTA||','||NU_MATRICULA||','||NU_CERTIFICADO||','''||NM_FUNDO||''','''||CD_FUNDO||''','''||CT_FUNDO||''','''||DT_MOVIMENTO||''','||QT_COTAS||','||VL_MOVIMENTO||','''||DT_COTA||''','||VL_COTA||','||VL_DA||','||VL_IR||','||NU_DOCUMENTO||','''||DT_PAGAMENTO||''','||NU_PARCELA||','||VALOR_TAXA_SAIDA||','''||DS_OPERACAO_REALIZADA||''','''||CD_ANGARIADOR||''','''||DS_AGENCIA||''','''||VL_IDENTIFICACAO_CLIENTE||''','''||NM_PRODUTO||''','''||CD_PLANO_PARCEIRO||''','||DECODE(NVL(NU_PROTOCOLO,0),0,'NULL',NU_PROTOCOLO)||','||VL_NOMINAL||','''||DS_CGC_CPF_CLIENTE||''','''||CD_MOVIMENTO_ICATU||''','''||DT_SOLICITACAO||''','''||NU_PROC_SUSEP||''','''||DS_SINAL_MOVIMENTACAO||''','''||NU_CONTRATO||''');'
...
-- 41 - POPULA - TB_ARQ_STATUS_CARGA_LOG 
